//! CRC8 Dallas/Maxim (polynomial 0x31, init 0x00)

const TABLE: [u8; 256] = [
    ,
];

/// Compute CRC8 Dallas/Maxim over a byte slice.
pub fn crc8(data: &[u8]) -> u8 {
    let mut crc: u8 = 0;
    for &b in data {
        crc = TABLE[(crc ^ b) as usize];
    }
    crc
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn empty() { assert_eq!(crc8(&[]), 0x00); }

    #[test]
    fn single_zero() { assert_eq!(crc8(&[0x00]), 0x00); }

    #[test]
    fn single_0x01() { assert_eq!(crc8(&[0x01]), 0x31); }

    #[test]
    fn deterministic() {
        let data = [0x47, 0x46, 0x01, 0x02];
        assert_eq!(crc8(&data), crc8(&data));
    }

    #[test]
    fn different_data() {
        assert_ne!(crc8(&[1, 2, 3]), crc8(&[1, 2, 4]));
    }
}
